...
export default function ModelList() {
    ...
    // const decodedModelName = decodeURIComponent(brand_name as string);
    ...
    const brand = carData.popular_checks.find(brand => brand.name === decodedBrandName)
    if(!brand) return <div>Brand not found</div> 

    const totalPages = Math.ceil(brand.models.length / itemsPerPage);
    const paginatedModels = brand.models.slice(
            ...
            <div className="vehicles mt-12 flex flex-col gap-4">
                ...
                {paginatedModels.map((a_brand, index) => (
                    <Link href={`/popular_checks/${encodeURIComponent(decodedBrandName)}/${encodeURIComponent(a_brand.model_name)}`} key={index}  className="vehicle inline-flex text-center group cursor-pointer">
                        <div className="py-2 text-xs md:text-sm lg:text-base font-montserrat w-2/5 font-medium bg-[#FFC439] text-white group-hover:bg-green-600 group-hover:text-white">{a_brand.model_name}</div>
                        <div className="py-2 text-xs md:text-sm lg:text-base font-raleway w-3/5 font-light border !border-s-none group-hover:bg-green-600 group-hover:text-white">{a_brand.vehicle_num} Vehicles have been Inspected</div>
                    </Link>
                ))}
