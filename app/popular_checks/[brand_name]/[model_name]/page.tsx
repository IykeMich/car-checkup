            ...
            <div className="vehicles mt-12 flex flex-col gap-4">
                ...
                {paginatedModelList.map((list, index) => (
                    <Link href={`/report`} key={index}  className="vehicle inline-flex text-center group cursor-pointer">
                        <div className="py-2 text-sm lg:text-base font-montserrat w-2/5 font-medium bg-[#FFC439] text-white group-hover:bg-green-600 group-hover:text-white">{list.year}</div>
                        <div className="py-2 text-sm lg:text-base font-raleway w-3/5 font-light border !border-s-none group-hover:bg-green-600 group-hover:text-white">{list.reg_number}</div>
                    </Link>
                ))}
