            ...
            <div className="vehicles mt-12 flex flex-col gap-4">
                ...
                {paginatedData.map((vehicle) => (
                    <Link href={`/popular_checks/${vehicle.name}`} key={vehicle.brand_id}  className="vehicle inline-flex text-center group cursor-pointer">
                        <div className="py-2 text-xs md:text-sm lg:text-base font-montserrat w-2/5 font-medium bg-[#FFC439] text-white group-hover:bg-green-600 group-hover:text-white">{vehicle.name}</div>
                        <div className="py-2 text-xs md:text-sm lg:text-base font-raleway w-3/5 font-light border !border-s-none group-hover:bg-green-600 group-hover:text-white">{vehicle.vehicle_num} Vehicles have been Inspected</div>
                    </Link>
                ))}
